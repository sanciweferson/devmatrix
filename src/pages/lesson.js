...
import {
  content as fundamentos01,
  init as initFundamentos01,
} from "@content/fundamentos/01-introducao"
...
import {
  content as constTipos03,
  initConst,
} from "@content/variaveis-tipos/03-const"
import {
  content as varLetConstTipos04,
  initVarLetConst,
} from "@content/variaveis-tipos/04-var-let-const"
import {
  content as tiposDeDadosTipos05,
  initTiposDeDados,
} from "@content/variaveis-tipos/05-tipos-de-dados"
import {
  content as primitivosTipos06,
  initPrimitivos,
} from "@content/variaveis-tipos/06-primitivos"
import {
  content as referenciaTipos07,
  initReferencia,
} from "@content/variaveis-tipos/07-referencia"
import {
  content as coercaoTipos08,
  initCoercao,
} from "@content/variaveis-tipos/08-coercao"
...
const INIT_MAP = {
  "fundamentos/01-introducao": initFundamentos01,
...
export function initLesson() {
  ...
  const aplicarFeedback = (copyBtn) => {
    ...
    setTimeout(() => {
      ...
      if (label) {
        label.textContent = copyBtn.classList.contains(
          "code-block__copy--console",
        )
          ? "Copiar tudo"
          : "Copiar"
      }
  ...
  document
    .querySelectorAll(".lesson__content .code-block__copy--console")
    .forEach((copyBtn) => {
      copyBtn.addEventListener("click", async () => {
        const block = copyBtn.closest(".code-block")
        if (!block) return
        ...
        const linhas = Array.from(block.querySelectorAll(".code-console__line"))
          .map((linha) => {
            const prompt =
              linha
                .querySelector(".code-console__prompt")
                ?.textContent.trim() ?? "›"
            const expr =
              linha.querySelector(".code-console__expr")?.textContent.trim() ??
              ""
            const output =
              linha
                .querySelector("[class*='syn-output']")
                ?.textContent.trim() ?? ""
